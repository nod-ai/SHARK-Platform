...
class mmt_scaled_q8_test(unittest.TestCase):
    ...
    def testBS32(self, atol, rtol):
        ...
        rhs_dype = torch.int8
        lhs = (torch.rand([16, 3200], dtype=ref_dtype)).to(torch.int8)
        rhs = (torch.rand([100, 3200], dtype=ref_dtype)).to(torch.int8)
        scale0 = (torch.rand([16], dtype=ref_dtype)).to(torch.int8)
        scale1 = (torch.rand([100], dtype=ref_dtype)).to(torch.int8)
        zp0 = (torch.rand([16], dtype=ref_dtype)).to(torch.int8)
        zp1 = (torch.rand([100], dtype=ref_dtype)).to(torch.int8)
        ...
        for i in range(lhs.shape[0]):
            scaled_lhs[i] = (lhs[i].to(ref_dtype) - zp0[i]) * scale0[i].to(ref_dtype)
        for i in range(rhs.shape[0]):
            scaled_rhs[i] = (rhs[i].to(ref_dtype) - zp1[i]) * scale1[i].to(ref_dtype)
