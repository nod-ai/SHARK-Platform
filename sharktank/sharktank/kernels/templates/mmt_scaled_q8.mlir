...
%mmt = linalg.generic {indexing_maps = [#map0, #map1, #map7, #map8, #map2], iterator_types = ["parallel", "parallel", "reduction"]}
ins(%arg0, %arg1, %arg4, %arg5 : tensor<{{m}}x{{k}}x{{lowp_type}}>, tensor<{{n}}x{{k}}x{{lowp_type}}>, tensor<{{m}}x{{lowp_type}}>, tensor<{{n}}x{{lowp_type}}>) outs(%fullf32 : tensor<{{m}}x{{n}}x{{a_type}}>) {
...
%empty = tensor.empty() : tensor<{{m}}x{{n}}x{{a_type}}>
    %result = linalg.generic {indexing_maps = [#map4, #map5, #map6, #map4], iterator_types = ["parallel", "parallel"]}
ins(%mmt, %arg2, %arg3 : tensor<{{m}}x{{n}}x{{a_type}}>, tensor<{{m}}x{{lowp_type}}>, tensor<{{n}}x{{lowp_type}}>) outs(%empty : tensor<{{m}}x{{n}}x{{a_type}}>) {
